...
```{r}
popular_cate_US <- c("Entertainment", "Music", "Howto & Style", "Comedy", "People & Blogs")
...
US_data %>%
  ...
  group_by(trending_date, category_name) %>%
  mutate(likes = mean(likes), dislikes = mean(dislikes), views = mean(views)) %>%
  gather(key = type, value = num, views, likes, dislikes) %>%
  ggplot(aes(x = trending_date, y = num, color = type)) +
  geom_point()+
  stat_smooth() + 
