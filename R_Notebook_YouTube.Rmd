...
In order to analysis the montly trend, the dataset will need to be splitted into monthly pieces. Then, the title should be splitted into words and add up the counts of each word used in the dataset.
...
prep_n_pron <- "(the|a|s|at|to|of|with|in|on|and|i|you|for|is|am|are|my|it|ft|from|your|thisup|by|his|off)"

### List of most popular words
pattern <- "(official|2017|2018|ft|how|christmas|trailer|live|audio)"
US_data %>%
  filter(grepl(pattern = pattern, ignore.case = TRUE, title)) %>%
  tidyr::extract(title, into = "pattern", regex = pattern) %>%
  group_by(category_name, pattern) %>%
  na.omit() %>%
  summarise(count = n()) %>%
  arrange(desc(count)) #%>%
#  ggplot(aes(x = year, y = category_name)) +
#  geom_point(aes(size = count, color = pattern))
```
