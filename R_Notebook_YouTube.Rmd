...
US_sub_data <- # small data with 10k rows
  US_data %>%
  head(10000)
### Load South Korean videos data from KRvideos.csv
...
KR_category_table <- fromJSON(file = "KR_category_id.json") #table of category value for South Korea
```

### Data Table Inspection
To start with, the main table with video information for US and South Korea is shown below:
```{r include=FALSE, paged.print=FALSE}
### US videos dataset
US_data %>%
  glimpse() #the glimpse function will present the columns down the page and rows across

### South Korean videos dataset
KR_data %>%
  glimpse()
...
### South Korea category datatable
print("South Korea category datatable")
KR_category_table %>%
  glimpse()
```
...
Moreover, for the category datasets, it they look a little messy. There are a 32 levels for US category datasets and 31 levels for the South Korea category dataset. Further cleaning of the data table will be conducted in later steps.
...
### Select variables for the South Korea dataset
KR_data <-
  KR_data %>%
...
### Convert variables for the South Korea video dataset
KR_data <-
  KR_data %>%
  mutate(trending_date = ydm(trending_date), category_id = as.character(category_id))
```
...
### Step 1: Most popular video categories for US and South Korea
```{r warning=FALSE}
...
### Most Popular Categories for South Korea
KR_data %>%
#  group_by(category_name) %>%
#  summarise(count = n())%>%
#  arrange(desc(count)) %>%
  ggplot(aes(x = category_name, fill = category_name)) +
  geom_bar(stat = "count") +
  xlim(c("Entertainment", "News & Politics", "People & Blogs", "Film & Animation", "Comedy", "Music", "Gaming", "Sports", "Pets & Animals", "Howto & Style", "Education"))+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

```
According to the two plots, among all trending videos, Entertainment videos is the most common for both countries. Moreover, Music and Howto & Style videos appears to be more common in the US while videos about news and politics as well as People & Blogs appear to be more common among trending videos on YouTube in South Korea.
...
### Define Top categories table of the South Korea
...
According to the plot, the differences in most common categories of trending videos between US and South Korea seem appearant. Similar to our observation above, the plot shows that overall, the most common video categories in South Korea is Entertainment, News & Politics, and Peopl & Blogs, while in US is Entertainment, Music, and Howto & Styles. 

However, it also shows the change among time. In the subplot for South Korea, the percent of count for Entertainment, News & Politics, and Peopl & Blogs started with similar values. However, by the middle of June, there is an increase trend in Entertainment videos percent count. Also, there is a decreasing pattern in People & Blog videos percent count since the beginning of June. 
...
# Section: If including popular phrases, will the videos receive higher views?
```{r}
popular_cate_US <- c("Entertainment", "Music")
### List of most popular words
pattern <- "(2017|2018|christmas|how|live|makeup|music|new|super|trailer|vs|what|bowl)"
US_data %>%
#  mutate(case = ifelse(grepl(pattern = pattern, ignore.case = TRUE, title), "With Pattern", "Without Pattern")) %>%
...
This plot shows that in general, entertainment videos with at least one of the vocabularies (2017, 2018, chistmas, how, music, live, makeup, music, new, super, trailer, what, vs, bowl) in the tile has higher mean views than videos without the vocaularies. This is especially obvious in December, and after middle March. In order to state with more confidence whether this difference is significant, error bars will be plotted for each day.

However, for music videos, the line plots for videos with and without patterns overlap a lot. This indicates music videos with and without these vocabularies appears to be similar in most of the time. This make sense because most of these vocabularies do not seem to bring much attraction to music videos.
...
In this plot, each vertical line indicates 93.3% confidence interval bands. 

It is interesting to see that although YouTube entertainment videos with the patterns have higher average views, the variance for these videos are larger than the videos without the patterns. This result in overlaping confidence intervals for most of the time, which lead to lack of significance to conclude that videos with pattern will have higher views on average. However, for some period from late April to early June, the difference bettwen views of videos with pattern in title appears to be significant different from the ones without the pattern. 

Moreover, the plot for music videos agrees with observations in the previous plot. The intervals for music with and without patterns overlap for most of the time. The only period that videos with pattern has higher average views is at around end of May.
...
US_data %>%
  ...
  summarise(count = n(), view = sum(views), likes = sum(likes), dislikes = sum(dislikes)) %>%
  arrange(desc(count))
```
